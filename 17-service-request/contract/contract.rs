...
#[contracttype]
#[derive(Clone)]
pub struct ServiceRequestContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum ServiceRequestContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum ServiceRequestContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl ServiceRequestContract {
    fn count_key() -> ServiceRequestContractDataKey {
        ServiceRequestContractDataKey::Count
    }

    fn ids_key() -> ServiceRequestContractDataKey {
        ServiceRequestContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> ServiceRequestContractDataKey {
        ServiceRequestContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, ServiceRequestContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, ServiceRequestContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn request_item(env: Env, id: Symbol, owner: Address, title: String, notes: String, state: Symbol, amount: i128, updated_at: u64) {
        ...
        let item = ServiceRequestContractItem {
            owner,
            title,
            notes,
            state,
    ...
    pub fn accept_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<ServiceRequestContractItem> = env.storage().instance().get(&key);
    ...
    pub fn complete_item(env: Env, id: Symbol) -> Option<ServiceRequestContractItem> {
