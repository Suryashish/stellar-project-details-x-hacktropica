...
#[contracttype]
#[derive(Clone)]
pub struct VotingPollingContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum VotingPollingContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum VotingPollingContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl VotingPollingContract {
    fn count_key() -> VotingPollingContractDataKey {
        VotingPollingContractDataKey::Count
    }

    fn ids_key() -> VotingPollingContractDataKey {
        VotingPollingContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> VotingPollingContractDataKey {
        VotingPollingContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, VotingPollingContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, VotingPollingContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn create_item(env: Env, id: Symbol, owner: Address, title: String, notes: String, state: Symbol, amount: i128, updated_at: u64) {
        ...
        let item = VotingPollingContractItem {
            owner,
            title,
            notes,
            state,
    ...
    pub fn vote_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<VotingPollingContractItem> = env.storage().instance().get(&key);
    ...
    pub fn tally_item(env: Env, id: Symbol) -> Option<VotingPollingContractItem> {
