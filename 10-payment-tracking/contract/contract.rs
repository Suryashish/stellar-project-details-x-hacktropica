...
#[contracttype]
#[derive(Clone)]
pub struct PaymentTrackingContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum PaymentTrackingContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum PaymentTrackingContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl PaymentTrackingContract {
    fn count_key() -> PaymentTrackingContractDataKey {
        PaymentTrackingContractDataKey::Count
    }

    fn ids_key() -> PaymentTrackingContractDataKey {
        PaymentTrackingContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> PaymentTrackingContractDataKey {
        PaymentTrackingContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, PaymentTrackingContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, PaymentTrackingContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn record_item(env: Env, id: Symbol, owner: Address, title: String, notes: String, state: Symbol, amount: i128, updated_at: u64) {
        ...
        let item = PaymentTrackingContractItem {
            owner,
            title,
            notes,
            state,
    ...
    pub fn verify_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<PaymentTrackingContractItem> = env.storage().instance().get(&key);
    ...
    pub fn settle_item(env: Env, id: Symbol) -> Option<PaymentTrackingContractItem> {
