...
#[contracttype]
#[derive(Clone)]
pub struct GeoAlertContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum GeoAlertContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum GeoAlertContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl GeoAlertContract {
    fn count_key() -> GeoAlertContractDataKey {
        GeoAlertContractDataKey::Count
    }

    fn ids_key() -> GeoAlertContractDataKey {
        GeoAlertContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> GeoAlertContractDataKey {
        GeoAlertContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, GeoAlertContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, GeoAlertContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn trigger_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<GeoAlertContractItem> = env.storage().instance().get(&key);
    ...
    pub fn subscribe_item(env: Env, id: Symbol) -> Option<GeoAlertContractItem> {
