...
#[contracttype]
#[derive(Clone)]
pub struct PayToUnlockContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum PayToUnlockContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum PayToUnlockContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl PayToUnlockContract {
    fn count_key() -> PayToUnlockContractDataKey {
        PayToUnlockContractDataKey::Count
    }

    fn ids_key() -> PayToUnlockContractDataKey {
        PayToUnlockContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> PayToUnlockContractDataKey {
        PayToUnlockContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, PayToUnlockContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, PayToUnlockContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn list_item(env: Env, id: Symbol, owner: Address, title: String, notes: String, state: Symbol, amount: i128, updated_at: u64) {
        ...
        let item = PayToUnlockContractItem {
            owner,
            title,
            notes,
            state,
    ...
    pub fn purchase_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<PayToUnlockContractItem> = env.storage().instance().get(&key);
    ...
    pub fn unlock_item(env: Env, id: Symbol) -> Option<PayToUnlockContractItem> {
