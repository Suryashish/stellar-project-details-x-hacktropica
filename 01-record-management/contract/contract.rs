...
#[repr(u32)]
pub enum RecordManagementContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl RecordManagementContract {
    fn count_key() -> RecordManagementContractDataKey {
        RecordManagementContractDataKey::Count
    }
    ...
    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, RecordManagementContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, RecordManagementContractError::InvalidTimestamp);
        }
    }
