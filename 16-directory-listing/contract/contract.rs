...
#[contracttype]
#[derive(Clone)]
pub struct DirectoryListingContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum DirectoryListingContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum DirectoryListingContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl DirectoryListingContract {
    fn count_key() -> DirectoryListingContractDataKey {
        DirectoryListingContractDataKey::Count
    }

    fn ids_key() -> DirectoryListingContractDataKey {
        DirectoryListingContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> DirectoryListingContractDataKey {
        DirectoryListingContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, DirectoryListingContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, DirectoryListingContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn register_item(env: Env, id: Symbol, owner: Address, title: String, notes: String, state: Symbol, amount: i128, updated_at: u64) {
        owner.require_auth();
        Self::validate_item(&env, &title, updated_at);

        let item = DirectoryListingContractItem {
            owner,
            title,
            notes,
            state,
    ...
    pub fn search_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<DirectoryListingContractItem> = env.storage().instance().get(&key);
    ...
    pub fn update_item(env: Env, id: Symbol) -> Option<DirectoryListingContractItem> {
