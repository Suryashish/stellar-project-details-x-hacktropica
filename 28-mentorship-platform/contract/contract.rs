...
#[repr(u32)]
pub enum MentorshipPlatformContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl MentorshipPlatformContract {
    fn count_key() -> MentorshipPlatformContractDataKey {
        MentorshipPlatformContractDataKey::Count
    }
    ...
    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, MentorshipPlatformContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, MentorshipPlatformContractError::InvalidTimestamp);
        }
    }
