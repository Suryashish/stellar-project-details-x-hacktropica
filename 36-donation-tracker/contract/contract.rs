...
#[contracttype]
#[derive(Clone)]
pub struct DonationTrackerContractItem {
    pub owner: Address,
    pub title: String,
    pub notes: String,
    pub state: Symbol,
...
#[contracttype]
#[derive(Clone)]
pub enum DonationTrackerContractDataKey {
    IdList,
    Item(Symbol),
    Count,
}
...
#[repr(u32)]
pub enum DonationTrackerContractError {
    InvalidTitle = 1,
    InvalidTimestamp = 2,
}
...
impl DonationTrackerContract {
    fn count_key() -> DonationTrackerContractDataKey {
        DonationTrackerContractDataKey::Count
    }

    fn ids_key() -> DonationTrackerContractDataKey {
        DonationTrackerContractDataKey::IdList
    }

    fn item_key(id: &Symbol) -> DonationTrackerContractDataKey {
        DonationTrackerContractDataKey::Item(id.clone())
    }

    fn validate_item(env: &Env, title: &String, updated_at: u64) {
        if title.len() == 0 {
            panic_with_error!(env, DonationTrackerContractError::InvalidTitle);
        }

        if updated_at == 0 {
            panic_with_error!(env, DonationTrackerContractError::InvalidTimestamp);
        }
    }
    ...
    pub fn donate_item(env: Env, id: Symbol, owner: Address, title: String, notes: String, state: Symbol, amount: i128, updated_at: u64) {
        owner.require_auth();
        Self::validate_item(&env, &title, updated_at);

        let item = DonationTrackerContractItem {
            owner,
            title,
            notes,
            state,
    ...
    pub fn track_item(env: Env, id: Symbol, state: Symbol, notes: String, updated_at: u64) -> bool {
        let key = Self::item_key(&id);
        let maybe_item: Option<DonationTrackerContractItem> = env.storage().instance().get(&key);
    ...
    pub fn verify_item(env: Env, id: Symbol) -> Option<DonationTrackerContractItem> {
